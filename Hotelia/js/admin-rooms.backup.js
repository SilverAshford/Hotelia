        ...
        <tr id="room-row-${r.id}">
            ...
            <td class="text-end px-2">
                <div class="d-flex justify-content-end gap-1 flex-wrap">
                    <button class="btn btn-sm btn-primary" onclick="openRoomManagement(${r.id}, currentAuth)" title="Manage room">
        ...
        mobileWrapper.innerHTML = rooms.map(r => {
            ...
            <div class="room-mobile-card mb-3 p-3" id="room-card-mobile-${r.id}">
                ...
                <div class="d-flex gap-2 pt-2 border-top">
                    <button class="btn btn-primary flex-fill" onclick="openRoomManagement(${r.id}, currentAuth)">
                        <i class="fa-solid fa-sliders me-1"></i>Manage
                    </button>
...
function injectModals() {
    if (!document.getElementById('roomManagementModal')) {
        document.body.insertAdjacentHTML('beforeend', `
        <div class="modal fade" id="roomManagementModal" tabindex="-1" aria-hidden="true">
            <div class="modal-dialog modal-xl">
                <div class="modal-content">
                    <div class="modal-header" style="background:var(--header-gradient);">
                        <div>
                            <h5 class="modal-title fw-bold text-white" id="managementModalTitle">
                                <i class="fa-solid fa-sliders me-2"></i>Room Management
                            </h5>
                            <p class="text-white opacity-75 small mb-0" id="managementModalSubtitle"></p>
                        </div>
                        <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal"></button>
                    </div>
                    <div class="modal-body p-0">
                        <!-- Tabs Navigation -->
                        <ul class="nav nav-tabs nav-fill border-bottom" style="background:var(--background-color);">
                            <li class="nav-item">
                                <button class="nav-link active fw-bold" id="editTab" onclick="switchTab('edit')">
                                    <i class="fa-solid fa-pen me-2"></i>Edit Details
                                </button>
                            </li>
                            <li class="nav-item">
                                <button class="nav-link fw-bold" id="availTab" onclick="switchTab('availability')">
                                    <i class="fa-solid fa-calendar-days me-2"></i>Availability & Bookings
                                </button>
                            </li>
                        </ul>
                        ...
                        <div class="p-4">
                                ...
                                <form id="roomForm" novalidate>
                                    ...
                                    <div class="row g-3">
                                        ...
                                        <div class="col-md-6">
                                            ...
                                            <input type="text" class="form-control" id="roomBedType" placeholder="e.g. King Bed">
                                        </div>
                                        <div class="col-md-6">
                            ...
                            <div id="availTabContent" style="display:none;">
                                <!-- Set availability -->
                                <div class="p-3 rounded-3 mb-4" style="background:var(--background-color);border:1px solid var(--border-color);">
                                    <h6 class="fw-bold mb-3" style="color:var(--primary-color);">
                                        <i class="fa-solid fa-calendar-plus me-2"></i>Set Availability
                                    </h6>
                                    <div class="row g-3 align-items-end">
                                        <div class="col-md-3">
                                        ...
                                        <div class="col-md-3">
                                            ...
                                            <input type="number" class="form-control" id="availTotalRooms" min="1" value="10">
                                        </div>
                                        <div class="col-md-3">
                                ...
                                <div class="d-flex justify-content-between align-items-center mb-3">
                                    <h6 class="fw-bold mb-0" style="color:var(--primary-color);">Current Availability</h6>
                                    <div class="d-flex gap-2">
                                        ...
                                        <button class="btn btn-sm btn-dark" id="refreshAvailBtn">
                                            <i class="fa-solid fa-rotate-right"></i>
                                        </button>
                                    </div>
                                </div>
                                <div class="table-responsive table-custom" style="max-height:300px;overflow-y:auto;">
                                    <table class="table table-hover mb-0">
                                        ...
                                        <tbody id="availTableBody">
                                            <tr><td colspan="5" class="text-center text-muted py-3">Set dates and click refresh</td></tr>
                                        </tbody>
                                    </table>
                                </div>
                                ...
                                <div class="mt-4 pt-3 border-top">
                                    <div class="d-flex justify-content-between align-items-center mb-3">
                                        <h6 class="fw-bold mb-0" style="color:var(--primary-color);">Active Bookings</h6>
                                        <button class="btn btn-sm btn-dark" id="refreshRoomBookingsBtn">
                                            <i class="fa-solid fa-rotate-right"></i>
                                        </button>
                                    </div>
...
function openAvailabilityPanel(roomTypeId, roomTypeName, maxBooked = 0) {
    ...
    document.getElementById('availModalSubtitle').textContent =
        `${roomTypeName} · ${selectedHotel?.name || ''} · ${room?.totalRooms ?? '?'} total · ${room?.availableRooms ?? '?'} available now`;
    ...
    const defaultTotal = room?.totalRooms ?? 10;
    const minTotal = Math.max(1, maxBooked || room?.maxBookedRooms || 0);
    ...
    const availTotalInput = document.getElementById('availTotalRooms');
    availTotalInput.value   = String(defaultTotal);
    availTotalInput.min     = String(minTotal);
...
async function loadRoomBookings() {
    ...
    listEl.innerHTML = `<div class="text-center text-muted py-3 small">
    ...
    try {
        ...
        if (!bookings.length) {
            listEl.innerHTML = `<div class="text-center text-muted py-3 small">No active bookings for this room type.</div>`;
            return;
        }
                ...
                <table class="table table-sm table-hover mb-0">
                    ...
                    <tbody>
                        ${bookings.map(b => {
                            ...
                            return `
                            <tr>
                                <td class="fw-bold text-muted">#BK-${b.id}</td>
                                <td>${b.userName}</td>
                                <td class="small">${formatDate(b.checkIn)} → ${formatDate(b.checkOut)}</td>
                                <td>${badge}</td>
                                <td class="text-end">
        ...
        listEl.innerHTML = `<div class="alert alert-warning py-2 mb-0 small">${err.message}</div>`;
    }
}
