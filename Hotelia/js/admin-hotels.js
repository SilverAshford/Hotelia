...
function renderHotelsTable(tbody, hotels) {
    if (!hotels.length) {
        ...
        if (mobileWrapper) {
            mobileWrapper.innerHTML = `<p class="text-center text-muted py-4">No hotels found. Add one!</p>`;
        }
        return;
    }
    ...
    tbody.innerHTML = hotels.map(h => {
        const imgSrc = h.thumbnailUrl || hotelThumbFromName(h.name);
        return `
        <tr id="hotel-row-${h.id}">
            ...
            <td>
                <div class="d-flex align-items-center gap-2">
                    <img src="${imgSrc}" alt="${h.name}"
                         style="width:52px;height:38px;object-fit:cover;border-radius:5px;flex-shrink:0;"
            ...
            <td class="text-end px-2">
                <div class="d-flex justify-content-end gap-1">
                    <button class="btn btn-sm btn-edit" onclick="openHotelModal(${h.id})" title="Edit">
                        <i class="fa-solid fa-pen"></i>
                    </button>
                    <button class="btn btn-sm btn-delete" onclick="deleteHotel(${h.id},'${escHtml(h.name)}')" title="Delete">
    ...
    mobileWrapper.innerHTML = hotels.map(h => {
        const imgSrc = h.thumbnailUrl || hotelThumbFromName(h.name);
        return `
        <div class="hotel-mobile-card mb-3 p-3" id="hotel-card-mobile-${h.id}">
            <div class="d-flex justify-content-between align-items-center mb-2 pb-2 border-bottom">
                <span class="fw-bold text-muted small">#HTL-${String(h.id).padStart(2,'0')}</span>
                <span class="badge bg-success">Active</span>
            </div>
            <div class="d-flex gap-3 align-items-center mb-2">
                <img src="${imgSrc}" alt="${h.name}" class="hotel-thumb-mobile rounded"
                     style="object-fit:cover;"
                     onerror="this.src='assets/hotel.jpg'">
                <div>
                    <h5 class="fw-bold mb-1">${h.name}</h5>
                    <p class="text-muted small mb-1">
                        <i class="fa-solid fa-location-dot me-1"></i>${h.city}${h.address ? ', ' + h.address : ''}
                    </p>
                    <p class="small mb-0">
                        <i class="fa-solid fa-star text-warning me-1"></i>${h.stars} Stars
                    </p>
                </div>
            </div>
            <div class="d-flex gap-2 mt-3 pt-2 border-top">
                <button class="btn btn-edit flex-fill" onclick="openHotelModal(${h.id})">
                    <i class="fa-solid fa-pen me-1"></i>Edit
                </button>
                <button class="btn btn-delete flex-fill" onclick="deleteHotel(${h.id},'${escHtml(h.name)}')">
...
window.saveImageToDisk = saveImageToDisk;
...
/**
 * Derives thumbnail path from hotel name — same convention used by saveImageToDisk():
